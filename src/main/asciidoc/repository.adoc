...
In the case where the index creation cost isn't considered too high, it can be triggered automatically instead. You just need to annotate the repositories you want managed with the relevant annotation(s):
 ...
 - `@N1qlSecondaryIndexed` will create a N1QL index that does the same kind of filtering on entity type. You can ensure a PRIMARY INDEX is there as well with this annotation.
 - `@N1qlPrimaryIndexed` can be used instead if you don't expect a large number of documents (low enough that a dedicated secondary index isn't needed) but just want to make sure a PRIMARY INDEX in N1QL is available.
